...
const createTable = `CREATE TABLE Directories (
...
var _ directory.Storage = &Directories{}

// Directories gives access to the directory table.
type Directories struct {
	db *ktspanner.Database
}

// New returns a directory.Storage client backed by Spanner.
func New(db *ktspanner.Database) *Directories {
	return &Directories{db: db}
}
...
func (d *Directories) List(ctx context.Context, showDeleted bool) ([]*directory.Directory, error) {
	client, err := d.db.Get(ctx)
	if err != nil {
		return nil, err
	}
	rtx := client.Single()
	...
	err = rtx.Read(ctx, table, spanner.AllKeys(), directoriesCols).
...
func (d *Directories) Write(ctx context.Context, dir *directory.Directory) error {
	client, err := d.db.Get(ctx)
	if err != nil {
		return err
	}
	...
	if err != nil {
		return fmt.Errorf("InsertStruct(): %w", err)
	}
...
func (d *Directories) Read(ctx context.Context, directoryID string, showDeleted bool) (*directory.Directory, error) {
	client, err := d.db.Get(ctx)
	if err != nil {
		return nil, err
	}
...
func (d *Directories) SetDelete(ctx context.Context, directoryID string, isDeleted bool) error {
	client, err := d.db.Get(ctx)
	if err != nil {
		return err
	}
...
func (d *Directories) Delete(ctx context.Context, directoryID string) error {
	client, err := d.db.Get(ctx)
	if err != nil {
		return err
	}
