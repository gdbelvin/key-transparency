...
func TestDDL(t *testing.T) {
	if _, err := spansql.ParseDDLStmt(createTable); err != nil {
		t.Fatal(err)
	}
}
...
func NewForTest(ctx context.Context, t *testing.T) (directory.Storage, func(context.Context)) {
	t.Helper()
	client, adminClient, cleanup := testutil.Client(ctx, t)
	if err := testutil.UpdateDDL(ctx, t, adminClient, createTable); err != nil {
		t.Fatal(err)
	}
